...
const getUser = async () => {
  try {
    const res = await axios.get(`http://192.168.0.109:8000/api/ordendetalles_index?userID=${userID}`, {
      headers: {
...
const getPrice = async () => {
  try {
    const res = await axios.get(`http://192.168.0.109:8000/api/precio?userID=${userID}`, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'Accept': 'application/json'
      }
...
const deleteProduct = async (productId) => {
  try {
    const res = await axios.post(`http://192.168.0.109:8000/api/delete_ordendetalles/${productId}`, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'Accept': 'application/json'
      }
